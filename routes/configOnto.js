...
router.post('/', function(req, res) {
  ...
  jsonfile.readFile(filepath, function(err, obj) {
    ...
    if (userData.repositoryType === "private") {
      ...
      var isRepoCloned = shell.exec('git clone https://"' + userData.user +
        ":" + encodeURIComponent(CryptoJS.AES.decrypt(userData.password
          .toString(), userData.user).toString(CryptoJS.enc.Utf8)) +
        "@" + userData.repositoryURL.slice(8) + '" repoFolder', {
          silent: false
        }).stdout;
      if (!isRepoCloned.includes("failed")) {
        ...
        if (isAdminPasswordModified || isPrivateLoginPasswordModified) {
          ...
          if (isPrivateLoginPasswordModified)
            bcrypt.hash(userData.loginPassword, salt, function(err,
              hash) {
