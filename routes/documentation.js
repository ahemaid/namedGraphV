...
var SKOSConceptsQuery2 = function(namedGraphsString) {
  return ("  PREFIX skos: <http://www.w3.org/2004/02/skos/core#> " +
    ...
    namedGraphsString + "  WHERE { " +
    "  GRAPH ?g { ?s ?p ?o }  GRAPH ?g { { ?subject skos:narrower ?oNarrower .  " +
        ...
        if (data != null) {
          ...
          if (graphs[0] != null) {
            ...
            for (var i in list) {
              ...
              if (list[i].includes(req.params.instanceID + '/' + req.params
                    .branchName + '/')) {
                namedGraphsString4Qurery += "from named <" + list[i] +
                  ">\n";
                filesArray.push(list[i].split(
                  req.params.instanceID + '/' + req.params.branchName +
                  '/')[
                  1])
              }
            }
